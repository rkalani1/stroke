...
- **Institution-neutral & public.** `rkalani1.github.io/stroke` carries **zero** named-institution content. (Already verified clean on main.)
...
> **Decision 2026-05-29:** "Protocols & Algorithms" is **not** a new separate tab — it is the **new title of the existing Management section**, fully named **"Institutional Protocols and Algorithms."** Public site shows the **real protocol content, de-identified** (no named-institution or identifying references — a reader can't tell whose protocol it is), satisfying the institution-neutral constraint while staying clinically substantive. The word "Institutional" is a generic descriptor; no institution is named.

- **Research & Guidelines** (public) — *"what the literature shows"* (teach from this): What's New feed + Evidence Atlas + guidelines.
- **Institutional Protocols & Algorithms** (renamed Management, public) — *"what we do"* patterns: the **real protocols, de-identified** (no named-institution or identifying refs); each links to its evidence basis. Visually distinct (gold).
- **Fully-branded institutional layer** — the *identified* source originals (named contacts, pagers, order-sets), **local-only, gitignored, never public** — the on-device source the public de-identified version is derived from. Built last.
...
3. **Institutional Protocols & Algorithms** — rename the Management section + present real protocols **de-identified** for the public build (no named-institution or identifying refs).
