...
` /stroke/` remains the primary distribution
...
- App Store Connect access for `app.stroke.demo`
...
- **Network usage**: explain in App Privacy nutrition label that the app
  ...
  OpenEvidence, UpToDate, Asta, Regional Facilities); no PHI is
  transmitted.
...
|---|---|---|
| **PWABuilder.com** | hours | Generates store packages straight from ` /stroke/` with no Capacitor in your repo. Worth trying first — if it produces an acceptable iOS/Android binary, skip Capacitor entirely. |
...
- 22+ calculators, public-demo persistence/census guardrails, hash routing,
...
The PWA at ` /stroke/` continues to serve all
