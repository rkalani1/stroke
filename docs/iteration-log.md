...
- Added automated latency-threshold suggestion generator:
  - new script: `./scripts/qa-latency-threshold-suggest.mjs`,
  - new command: `npm run qa:latency:suggest`,
  - new output artifact: `./docs/qa-latency-threshold-suggestions.md`.
- Extended ops index generator for latency-governance outputs:
  - updated `./scripts/evidence-ops-index.mjs` with recalibration and threshold-suggestions artifact/command coverage.
- Updated npm scripts:
  - `./package.json` now includes `qa:latency:suggest`.
...
- Consider legacy document-export library error handling improvements.
...
- Added legacy document-export library CDN load timeout guard (8s) to prevent indefinite hang on slow/unresponsive networks.
