...
const CorChip = ({ cor }) => {
  ...
  const color = norm.startsWith('1') ? 'bg-ok-100 text-ok-900 border-ok-300'
    ...
    : norm.includes('harm') ? 'bg-crit-100 text-crit-900 border-crit-400'
  ...
  return (
    <div className="p-3 rounded-lg border border-cobalt-300 bg-white">
      ...
      {state.hoursFromLKW && parseFloat(state.hoursFromLKW) > 4.5 && (
        <div className="mb-3 p-2 rounded border border-cobalt-200 bg-cobalt-50 text-xs">
          <p className="font-semibold text-cobalt-900 mb-1">Extended-window imaging selection (prefer MRI if small vessel, posterior, or contrast allergy):</p>
      ...
      <div className={`p-3 rounded border-2 ${colorByEligible(result.eligible)}`}>
        ...
        {Array.isArray(result.warnings) && result.warnings.length > 0 && (
          <ul className="list-disc list-inside text-xs text-warn-800 mt-1">
...
const EVTEligibilityCard = ({ defaults = {} }) => {
  ...
  const colorByEligible = (e) => e === true ? 'border-ok-400 bg-ok-50' : e === 'consider' ? 'border-yellow-400 bg-yellow-50' : e === 'pending' ? 'border-warn-400 bg-warn-50' : e === false ? 'border-rose-400 bg-rose-50' : 'border-slate-200';

  return (
    <div className="p-3 rounded-lg border border-cobalt-300 bg-white">
      <div className="flex items-center justify-between mb-2">
        <h4 className="font-bold text-cobalt-900 flex items-center gap-2">
        ...
        <div className="flex gap-1 flex-wrap">
          {['anterior', 'm2-distal', 'basilar'].map((k) => (
            <button key={k} type="button" onClick={() => setBranch(k)} className={`px-2 py-1 text-xs rounded ${branch === k ? 'bg-cobalt-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
        ...
        <>
          ...
          <div className={`p-2 rounded border-2 ${colorByEligible(rAnt.eligible)}`}>
            <div className="flex items-center flex-wrap gap-2">
              ...
              {rAnt.window && <span className="px-1.5 py-0.5 text-xs bg-cobalt-100 text-cobalt-900 rounded">{rAnt.window}</span>}
            </div>
      ...
      {branch === 'basilar' && (
        <>
          ...
          <div className={`p-2 rounded border-2 ${colorByEligible(rBas.eligible)}`}>
            ...
            {rBas.institutionalRequirement && <p className="text-xs mt-1 text-cobalt-800"><strong>Example institutional requirement:</strong> {rBas.institutionalRequirement}</p>}
          </div>
        </>
      )}
...
const ContraindicationsCard = () => (
  <div className="p-3 rounded-lg border border-crit-300 bg-white">
    <h4 className="font-bold text-crit-900 mb-2 flex items-center gap-2">
      <span className="inline-block px-2 py-0.5 bg-crit-700 text-white text-xs rounded">EX</span>
      IVT Contraindications
    </h4>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
      <section>
        <h5 className="font-bold text-crit-900 border-b border-crit-200 pb-1 mb-1">Absolute</h5>
      ...
      <section>
        <h5 className="font-bold text-warn-900 border-b border-warn-200 pb-1 mb-1">Relative (individualize)</h5>
        <ul className="space-y-1">
      ...
      <section>
        <h5 className="font-bold text-ok-900 border-b border-ok-200 pb-1 mb-1">Benefit &gt; Risk · Consider IVT</h5>
        <ul className="space-y-1">
...
const SafePauseCard = ({ defaults = {} }) => {
  ...
  return (
    <div className="p-3 rounded-lg border border-ok-300 bg-white">
      <h4 className="font-bold text-ok-900 mb-2 flex items-center gap-2">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
      <section>
        ...
        <table className="w-full">
          <tbody>
            <tr className="bg-ok-50"><td className="px-2 py-1 font-bold">Class 1 (Strong)</td><td className="px-2 py-1">Benefit &gt;&gt;&gt; Risk</td><td className="px-2 py-1">Is recommended</td></tr>
            ...
            <tr className="bg-crit-100"><td className="px-2 py-1 font-bold">Class 3: Harm</td><td className="px-2 py-1">Risk &gt; Benefit</td><td className="px-2 py-1">Avoid (harmful)</td></tr>
          </tbody>
        </table>
      </section>
      <section>
