  ...
  return (
    <div className="evd-sim space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Waveform panel */}
        <section className="bg-white border border-line rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-sm font-semibold text-slate-800">ICP Waveform</h4>
            <span className="font-mono text-2xs text-slate-500">P1 percussion · P2 tidal · P3 dicrotic</span>
          </div>
        ...
        {/* EVD apparatus SVG */}
        <section className="bg-white border border-line rounded-lg p-3">
          <h4 className="text-sm font-semibold text-slate-800 mb-2">EVD Apparatus &amp; Leveling</h4>
      ...
      {/* Controls */}
      <section className="bg-slate-50 border border-line rounded-lg p-3 space-y-3">
        <h4 className="text-sm font-semibold text-slate-800">Controls</h4>
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          ...
          <button type="button" onClick={() => setTransducerOffset((o) => clamp(o + 5, -15, 15))}
            className="px-3 h-9 min-h-[44px] sm:min-h-0 rounded-md text-sm font-semibold bg-slate-200 text-slate-800 hover:bg-slate-300 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-cobalt-500">
            Lower bed (+5 cm)
          </button>
          <button type="button" onClick={() => setTransducerOffset((o) => clamp(o - 5, -15, 15))}
            className="px-3 h-9 min-h-[44px] sm:min-h-0 rounded-md text-sm font-semibold bg-slate-200 text-slate-800 hover:bg-slate-300 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-cobalt-500">
      ...
      <section className="bg-white border border-line rounded-lg p-3 space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-slate-800">Scenarios</h4>
          ...
          {Object.values(SCENARIOS).map((sc) => (
            <button key={sc.id} type="button" onClick={() => loadScenario(sc.id)}
              className={cx('px-3 h-9 min-h-[44px] sm:min-h-0 rounded-md text-sm font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-cobalt-500',
                currentCase === sc.id ? 'bg-cobalt-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200')}>
              {sc.label}
            </button>
          ))}
        </div>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-slate-800">ICP-Crisis Escalation Pathway</h4>
          <span className="text-xs font-semibold text-cobalt-700">
            ...
            return (
              <div key={tier.id}
                className={cx('rounded-lg border p-3 transition-colors',
                  active ? 'border-cobalt-500 bg-cobalt-50 ring-2 ring-cobalt-500' : 'border-line bg-white')}>
                <div className="flex items-center justify-between mb-1">
                  <h5 className={cx('text-sm font-bold', active ? 'text-cobalt-800' : 'text-slate-800')}>{tier.name}</h5>
                  {active && <span className="text-2xs font-bold uppercase tracking-wide text-cobalt-700">Active</span>}
                </div>
                <p className="text-2xs uppercase tracking-wide font-semibold text-slate-500 mb-1.5">{tier.trigger}</p>
                <ul className="space-y-1">
                  {tier.items.map((it, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-slate-700">
...
function Readout({ label, value, unit, tone, hint }) {
  const toneText = tone === 'crit' ? 'text-crit-700' : tone === 'warn' ? 'text-warn-700' : tone === 'ok' ? 'text-ok-700' : 'text-slate-900';
  return (
    <div className="bg-white border border-line rounded-md px-3 py-2">
      <p className="text-2xs uppercase tracking-wide font-semibold text-slate-500">{label}</p>
