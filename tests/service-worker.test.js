...
function loadServiceWorker() {
  ...
  const context = {
    ...
    caches: {
      ...
      keys: async () => ['stroke-cache-v6-17-0', 'stroke-cache-v6-18-0', 'stroke-cache-v6-18-1'],
      delete: async (key) => {
...
describe('service worker update lifecycle', () => {
  ...
  it('stages updates without claiming clients during activate', async () => {
    ...
    expect(worker.deletedCaches).toContain('stroke-cache-v6-17-0');
    expect(worker.deletedCaches).toContain('stroke-cache-v6-18-0');
    expect(worker.deletedCaches).not.toContain('stroke-cache-v6-18-1');
    expect(worker.matchAllOptions).toContainEqual({ includeUncontrolled: true });
    expect(worker.postedMessages).toContainEqual({ type: 'sw-update-ready', version: '6.18.1' });
  });
  ...
  it('claims clients and requests reload for the current update message', async () => {
    ...
    expect(worker.matchAllOptions).toContainEqual({ includeUncontrolled: true });
    expect(worker.postedMessages).toContainEqual({ type: 'sw-claimed-reload', version: '6.18.1' });
  });
