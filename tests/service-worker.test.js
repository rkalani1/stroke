...
function loadServiceWorker() {
  ...
  const context = {
    ...
    caches: {
      ...
      keys: async () => ['stroke-cache-v6-11-11', 'stroke-cache-v6-12-0', 'stroke-cache-v6-13-0'],
      delete: async (key) => {
...
describe('service worker update lifecycle', () => {
  ...
  it('stages updates without claiming clients during activate', async () => {
    ...
    expect(worker.deletedCaches).toContain('stroke-cache-v6-12-0');
    expect(worker.deletedCaches).not.toContain('stroke-cache-v6-13-0');
    expect(worker.matchAllOptions).toContainEqual({ includeUncontrolled: true });
    expect(worker.postedMessages).toContainEqual({ type: 'sw-update-ready', version: '6.13.0' });
  });
  ...
  it('claims clients and requests reload for the current update message', async () => {
    ...
    expect(worker.matchAllOptions).toContainEqual({ includeUncontrolled: true });
    expect(worker.postedMessages).toContainEqual({ type: 'sw-claimed-reload', version: '6.13.0' });
  });
