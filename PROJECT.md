...
- **Data Layer (`data/`, `content/`)**: 89 guideline datasets (862 statements in `data/guidelines/`), 40 educational modules (`content/education/*.md`), 130 completed trials (`content/trials/`), 9 active trials (`src/evidence/activeTrials.js`), and citations registry (`src/evidence/citations.js`).
- **Logic & Matcher Engines (`src/evidence/`, `src/management-guidance.js`, `src/app.jsx`)**: 128 dynamic recommendation rules (`GUIDELINE_RECOMMENDATIONS`), 8 AIS Command Center Cards (`AIS_COMMAND_CENTER_CARDS`), 42 trial eligibility criteria evaluators with 14 exclusion guards (`src/evidence/matcher-engine.js`), and 2 protocol subtabs (`MANAGEMENT_SUBTABS` in `src/app.jsx`, backed by `src/institutional-protocols.js`).
...
| 7 | Guideline Library & COR/LOE Catalog | 89 guideline datasets (862 recs), search/filter by COR/LOE, page-level PDF publisher deep links | M1 | Survey (explorer_survey_1) |
| 8 | 40 Educational Markdown Modules | 40 `.md` modules in `content/education/` with YAML frontmatter, verified against `src/education.jsx` and `content/bundle.json` | M2 | Survey (explorer_survey_2) |
...
| 12 | Content Bundling Pipeline | `npm run content:bundle` bundling 233 records into deterministic `content/bundle.json` | M3 | Survey (explorer_survey_3) |
...
| 14 | Evidence & Matcher Validator | `npm run evidence:validate` verifying 9 active trials, 130 completed trials, 219 citations, 11 recs, 100% matcher engine coverage | M3 | Survey (explorer_survey_3) |
| 15 | Static Citation & Inline Citation Validators | `npm run validate:citations` (218 PMIDs checked) and `npm run validate:inline-citations` (263 unique PMIDs across 511 inline references) | M3 | Survey (explorer_survey_3) |
| 16 | Playwright Protocol Snapshot Lock | `npm run test:protocol-snapshot` locking 3 snapshot surfaces (ich, ischemic, calculators) with `npm run test:protocol-snapshot:update` baseline updater | M3 | Survey (explorer_survey_3) |
| 17 | Institutional & PHI Leak Guard | `npm run check:leak-guard` scanning 586 text files with 0 violations | M3 | Survey (explorer_survey_3) |
...
| M2 | Education Section & UX Accordion Verification | Verify all 40 educational modules, minimized `<details>` accordion containers, dark-mode AA contrast compliance, and 2024-2026 citations (Features 8-11) | M1 | DONE |
