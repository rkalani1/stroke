...
- **Data Layer (`data/`, `content/`)**: 17 guideline datasets (771 statements in `data/guidelines/`), 32 educational modules (`content/education/*.md`), 113 completed trials (`content/trials/`), 9 active trials (`src/evidence/activeTrials.js`), and citations registry (`src/evidence/citations.js`).
- **Logic & Matcher Engines (`src/evidence/`, `src/management-guidance.js`, `src/app.jsx`)**: 128 dynamic recommendation rules (`GUIDELINE_RECOMMENDATIONS`), 8 AIS Command Center Cards (`AIS_COMMAND_CENTER_CARDS`), 42 trial eligibility criteria evaluators with 13 exclusion guards (`src/evidence/matcher-engine.js`), and 6 institutional protocol subtabs (`src/institutional-protocols.js`).
...
| 14 | Evidence & Matcher Validator | `npm run evidence:validate` verifying 9 active trials, 113 completed trials, 87 citations, 11 recs, 100% matcher engine coverage | M3 | Survey (explorer_survey_3) |
