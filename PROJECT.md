...
- **UI & Presentation Layer (`src/`, `index.html`)**: Collapsible `<details>` accordion containers minimized by default, 5 interactive simulators (`EvdIcpSimulator`, `HintsSimulator`, `NeuroExamsTool`, `PupillometrySimulator`, `NihssSimulator`), dark mode design token system with WCAG AA compliance (21 verified token pairs).
